...
use App\Entities\{Collect, Util};

use App\Http\Requests\{ActivityCreateRequest, ActivityUpdateRequest};
use App\Repositories\{ActivityRepository, CancellationTypeRepository, CollectRepository, CollectorRepository};
        ...
        if (Auth::check()) {
            if (Auth::user()->type > 1) {
                ...
                if (Auth::user()->type > 2) {
                    ...
                    if ($request->has('dateConsult')) {
                        $dateNow = trim(Util::setDateLocalBRToDb($request->get('dateConsult'), false));
                    }
                } else {
