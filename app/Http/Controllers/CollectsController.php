...
use App\Repositories\{ActivityRepository, CancellationTypeRepository, CollectRepository, CollectorRepository, NeighborhoodRepository, PatientTypeRepository, PersonRepository, UserRepository};
...
class CollectsController extends Controller
{
    ...
    protected $cancellationTypeRepository;
    ...
    public function listAll(Request $request)
    {
        ...
        if (count($request->all()) > 0 && $request['value']) {
            if ($request->get('typeSearch') == 'collects.date') {
                $request['value'] = Util::setDateLocalBRToDb($request->get('value'), false);
            }
    ...
    public function listDone()
    {
        ...
        return view('collect.template_table', [
            ...
            'titlecard'  => 'Lista das últimas 500 coletas concluídas',
            //Info of entitie
            'table'               => $this->repository->getTable(),
            'thead_for_datatable' => ['Data/Hora', 'Código', 'Paciente', 'Pagamento Taxa', 'Bairro', 'Endereço', 'Coletador', 'Status'],
            'collect_list'        => $collect_list,
    ...
    public function schedule($id)
    {
        ...
        try {
            ...
            if (Auth::user()->type > 2) {
                $neighborhood_model = $this->neighborhoodRepository->find($collect->neighborhood_id);
            }
            ...
            if ($collect->status != 1 && $collect->status < 6) { // Se a coleta não estiver disponível ou cancelada ou confirmada
                ...
                if ($request->has('attachment')) {
                    ...
                    foreach ($request->allFiles()['attachment'] as $archives) {
                        if ($archives->getMimeType() == 'application/pdf' || $archives->getMimeType() == 'image/png' || $archives->getMimeType() == 'image/jpg' || $archives->getMimeType() == 'image/jpeg') {
                ...
                if ($request->has('cancellationType_id')) {
                    ...
                    if ($collect->extra != '1') {
                        ...
                        if ($reset) {
                            ...
                            Log::channel('mysql')->info('Reset: false');
                        }
                    }
    ...
    public function reserve(Request $request)
    {
        ...
        if ($collect->status > 1 || isset($collect->cancellationType_id) || isset($collect->neighborhood) || isset($collect->reserved_at)) {
            ...
            if ($this->repository->where([['collector_id', $collect->collector_id],
                ['date', $collect->date],
                ['id', '!=', $collect->id],
            ])
    ...
    public function sendconfirmation($id)
    {
        try {
            ...
            if ($status) {
                $this->repository->update(['sendconfirmation' => $collect->sendconfirmation + 1], $collect->id);
            }
        } catch (Exception $e) {
