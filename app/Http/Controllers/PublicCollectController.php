...
use App\Repositories\{CancellationTypeRepository, CollectRepository, CollectorRepository, FreeDayRepository, NeighborhoodRepository, PatientTypeRepository, PersonRepository};
...
class PublicCollectController extends Controller
{
            ...
            foreach ($collector_list as $collector) {
                foreach ($collector->neighborhoods as $neighborhood) {
                    if ($neighborhood->id == $neighborhood_id) {
                        array_push($array_collectors, $collector->id);
                    }
                }
            }
    ...
    public function schedule(Request $request, $id)
    {
        try {
            ...
            if ($quant > 2) {
                $price = ($quant - 1) * $collect->neighborhood->displacementRate;
            }
    ...
    public function reserve(Request $request)
    {
        try {
            ...
            if ($id_collect == 'Selecione') {
                return redirect()->route('public.index')->withErrors(['Selecione um horário']);
            }
            ...
            if ($id_collect != 'Selecione') {
                ...
                if ($request->session()->has('collect')) {
                    $sessionActive = $request->session()->get('collect');
                    if ($sessionActive->id != (int)$id_collect) {
                        return redirect()->route('public.index');
                    }
                }
